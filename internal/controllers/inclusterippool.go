...
func genericReconcile(ctx context.Context, c client.Client, pool pooltypes.GenericInClusterPool) (_ ctrl.Result, reterr error) {
	...
	if !pool.GetDeletionTimestamp().IsZero() {
		return reconcileDelete(ctx, pool, inUseCount)
	}
...
func reconcileDelete(ctx context.Context, pool pooltypes.GenericInClusterPool, addressesInUseCount int) (_ ctrl.Result, reterr error) {
	if addressesInUseCount == 0 {
